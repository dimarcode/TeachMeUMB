...
class Subject(db.Model):
    ...
    topic: so.Mapped[str] = so.mapped_column(sa.String(100), 
                                             unique=True, nullable=False)

    subject_appointments = db.relationship('Appointment', 
                                           foreign_keys='Appointment.subject_id', 
                                           backref='subject', lazy='dynamic')
    requested_subjects = db.relationship('RequestedSubject', 
                                         foreign_keys='RequestedSubject.subject_id',
...
class RequestedSubject(db.Model):
    ...
    def __repr__(self):
        return f"<RequestedSubject Subject ID: {self.subject_id}, \
...
class User(UserMixin, db.Model):
    ...
    user_requested_subjects = db.relationship('RequestedSubject', 
                                              foreign_keys='RequestedSubject.student_id', 
    ...
    student_appointments = db.relationship('Appointment', 
                                           foreign_keys='Appointment.student_id', 
                                           backref='student', lazy='dynamic')
    tutor_appointments = db.relationship('Appointment', 
                                         foreign_keys='Appointment.tutor_id', 
...
class Appointment(db.Model):
    ...
    created_date: so.Mapped[datetime] = so.mapped_column(default=func.now(), nullable=False)
    booking_date: so.Mapped[date] = so.mapped_column(nullable=False)
    booking_time = db.Column(db.Time, nullable=False, index=True)
